...
-- Constants`
--
local BROKER_NAME                      = "Broker_CombatTime_MoveSpeed_Mplus"
...
local combatTimePrefix                 = "combat "
local movementSpeedPrefix              = ""
...
local function onUpdateFallBack()
    debug(secretText)
    setText(secretText)
end


local function onUpdateMPlus()
    if not C_ChallengeMode.IsChallengeModeActive() or not C_ChallengeMode.GetActiveKeystoneInfo() then
        onUpdateFallBack()
        return
    end

    local mythicPlusKeyLevel, _, _ = C_ChallengeMode.GetActiveKeystoneInfo()

    if not mythicPlusKeyLevel then
        onUpdateFallBack()
        return
    end

    local text = string.format("M+%d", mythicPlusKeyLevel)
    debug(text)
    setText(text)
end
...
local function onAddonLoaded(addon)
    ...
    if success and result then
        ...
        ldb:NewDataObject(BROKER_NAME, {
            ...
            label = "CombatTime/RunSpeed/M+ Key Level",
        })
    end
end
